...
fn _berreman(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(pybind::linear_optics_scalar_py, m)?)?;
    m.add_function(wrap_pyfunction!(pybind::polarizance_decompose_py, m)?)?;
    m.add_function(wrap_pyfunction!(pybind::brown_params_py, m)?)?;
    m.add_function(wrap_pyfunction!(pybind::diff_mueller_matrix_py, m)?)?;
    m.add_function(wrap_pyfunction!(pybind::mueller_from_diff_py, m)?)?;
    m.add_function(wrap_pyfunction!(pybind::bulk_differential_py, m)?)?;
    m.add_function(wrap_pyfunction!(pybind::mueller_from_diff_stack_product_py, m)?)?;
